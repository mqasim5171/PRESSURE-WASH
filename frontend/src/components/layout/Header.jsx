...
// CSS variable (Admin > Appearance > Primary Color), hex is the fallback
// used until ThemeProvider has fetched a value - see index.css/ThemeProvider.jsx.
const CYAN = "var(--horizon-primary, #22d3ee)";

const Header = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [isAreasOpen, setIsAreasOpen] = useState(false);
  const [pastHero, setPastHero] = useState(false);
  // The nav is dark year-round, so it wants the light-on-dark logo variant
  // first, falling back to the general logo, then the static asset in
  ...
  // page content). Watching where the actual end of the hero (#hero-end)
  // sits relative to the floating navbar gets this right regardless of
  // hero length.
  //
  // This used to be an IntersectionObserver with a negative rootMargin
  // meant to shrink the observed area down to a thin strip at the very top
  // - but rootMargin's top value shrinks the root *inward from the top*,
  // it doesn't carve out a strip *at* the top, so the "observed" region was
  // actually most of the viewport (64px down to the bottom). Since the hero
  // is 92vh, #hero-end sits inside that region on the very first paint,
  // before any scrolling - so `pastHero` came out `true` immediately on
  // every load, which is exactly the "navbar looks solid over the hero
  // photo" behaviour reported. A direct geometric check against the
  // marker's own position is simpler and doesn't have that failure mode.
  useEffect(() => {
    if (!isHome) {
      setPastHero(false);
      return undefined;
    ...
    const NAV_THRESHOLD = 80; // px - roughly the floating navbar's own height + top inset
    let raf = null;
    const update = () => {
      raf = null;
      setPastHero(marker.getBoundingClientRect().top <= NAV_THRESHOLD);
    };
  ...
  const linkCls = "font-medium text-sm tracking-wide text-white/70 hover:text-white transition-colors whitespace-nowrap";

  const navContent = (
    <div className="max-w-7xl mx-auto flex items-center justify-between py-2 px-4 sm:px-6 mobile-nav-inner">
      ...
      <Link to="/" className="flex items-center space-x-2 sm:space-x-3">
        ...
        <img src={headerLogo} alt="" className="h-7 sm:h-8 w-auto mobile-logo-mark" />
        <span className="text-base sm:text-lg font-bold tracking-wider text-white mobile-wordmark">
          Horizon <span style={{ color: CYAN }}>Solar</span>
        </span>
      </Link>
      ...
      <nav className="hidden lg:flex items-center space-x-5 xl:space-x-7">
        ...
        <Link to="/#why-us" className={linkCls}>Why Us</Link>
        <Link to="/#packages" className={linkCls}>Packages</Link>
        ...
        <Link
          to="/contact"
          className="text-black px-4 py-2 rounded-full font-semibold text-sm hover:bg-white transition-colors whitespace-nowrap"
  ...
  const mobileMenu = isOpen && (
    <div className="lg:hidden bg-[#02060c]/98 backdrop-blur-xl border-t border-white/10">
      <nav className="flex flex-col space-y-3 px-6 py-6">
        ...
        <Link to="/#why-us" className={linkCls} onClick={() => setIsOpen(false)}>Why Us</Link>
        <Link to="/#packages" className={linkCls} onClick={() => setIsOpen(false)}>Packages</Link>
  ...
  if (isHome) {
    ...
    return (
      <header className="fixed w-full top-4 z-50 px-4 mobile-floating-header">
        <div
          // Frosted glass over the hero photo (light blur + faint border,
          // reads as floating glass rather than a solid bar) for as long as
          // the hero photo is actually behind it. Once scrolled past the
          // hero into flat dark page content, switches to the same solid
          // glass pill every other page uses, so it stays legible instead
          // of staying near-invisible over nothing.
          className={`max-w-7xl mx-auto rounded-2xl transition-all duration-300 mobile-nav-capsule ${
            pastHero
              ? "bg-[#02060c]/90 backdrop-blur-xl border border-white/10 mobile-nav-solid"
              : "bg-white/[0.06] backdrop-blur-sm border border-white/[0.10] mobile-nav-hero"
          }`}
        >
          {navContent}
        </div>
        ...
        {isOpen && (
          <div className="max-w-7xl mx-auto mt-2 rounded-2xl bg-[#02060c]/98 backdrop-blur-xl border border-white/10 lg:hidden overflow-hidden">
            {mobileMenu}
          </div>
        )}
      </header>
